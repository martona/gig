...
Launch `gig.exe`. A slim toolbar across the top shows the app name and live status (cameras up, frame rate, bandwidth, CPU) with gear / refresh / list icon buttons for **Settings**, **Reconnect**, and **Log** (hover for tooltips). The toolbar sits above the grid — cameras render in the area below it, undisturbed — and in single-camera zoom view it auto-hides so the image goes full-screen until you move the mouse. Each camera tile is labeled — click one to zoom it to fill the window, click again (or press Esc) to return to the grid. **Settings** (or **F2**) opens a settings dialog; **Reconnect** (or **F5**) re-logs-in, re-discovers, and rebuilds without restarting; **Log** toggles a full-window log view (Esc or the ✕ closes it). Esc in the grid quits. On first run (or unusable config) the settings dialog opens automatically. 
