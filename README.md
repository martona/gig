...
Launch `gig.exe`. A slim toolbar across the top shows the app name and live status (cameras up, frame rate, bandwidth, CPU) with gear / refresh / list icon buttons for **Settings**, **Reconnect**, and **Log** (hover for tooltips). The toolbar sits above the grid — cameras render in the area below it, undisturbed — and in single-camera zoom view it auto-hides so the image goes full-screen until you move the mouse. Each camera tile is labeled — click one to zoom it to fill the window, click again (or press Esc) to return to the grid. **Settings** (or **F2**) opens a native settings dialog applied live; **Reconnect** (or **F5**) re-logs-in, re-discovers, and rebuilds without restarting; **Log** toggles a full-window log view (Esc or the ✕ closes it). Esc in the grid quits. On first run (or unusable config) the settings dialog opens automatically. The old per-cell diagnostics tile is off by default now that the toolbar carries the status — re-enable it in Settings (the "Show diagnostics overlay" box) if you want the in-grid debug readout.
...
Server certificates are also checked against the requested **hostname**, and a **certificate pin** (trust-on-first-use) is offered if a server's cert can't be verified the normal way: a dialog shows the certificate details and its SPKI-SHA256 fingerprint, and pinning it (stored per host under `pins/` in the registry) trusts that exact certificate thereafter. If a pinned certificate later *changes*, a louder warning distinguishes a normal renewal from possible interception. `insecure` disables verification and pinning entirely.
